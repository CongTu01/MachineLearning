...
for i in range (1,np.shape(X_data)[1]+1):
    ...
    #id3
    clfID31 = tree.DecisionTreeClassifier(criterion="entropy").fit(X_train,y_train)
    y_predictID3 = clfID31.predict(X_test)
    #cart
    clfCart1 = tree.DecisionTreeClassifier(criterion="gini").fit(X_train,y_train)
    y_predictCart = clfCart1.predict(X_test)
    #svm
    svm1= SVC(kernel='linear').fit(X_train,y_train)
    y_predictsvm = svm1.predict(X_test)
    if(accuracy_score(y_test,y_predictID3)>max1):
        max1 = accuracy_score(y_test,y_predictID3)
        print(max1,"ID3","=>>",i)
        pca_best_id3 = pca
        modelMax_id3 = clfID31
    if(accuracy_score(y_test,y_predictCart)>max2):
        max2 = accuracy_score(y_test,y_predictCart)
        pca_best_cart = pca
        print(max2,"Cart","=>>",i)
        modelMax_cart = clfCart1
    if(accuracy_score(y_test,y_predictsvm)>max3):
        max3 = accuracy_score(y_test,y_predictsvm)
        print(max3,"SVM","=>>",i)
        pca_best_svm = pca
        modelMax_svm = svm1

#SVM
svm = SVC(kernel='linear').fit(X_train_first,y_train_first)


#ID3()
clfID3 = tree.DecisionTreeClassifier(criterion="entropy")
clfID3.fit(X_train_first,y_train_first)


#Cart()
clfCart = tree.DecisionTreeClassifier(criterion='gini')#entropy 
clfCart.fit(X_train_first,y_train_first)


lable_note = Label(form, text = "Khi chưa dùng PCA :",font=("Arial Bold", 10),fg="red")
lable_note.grid(row = 25, column = 1,padx = 40, pady = 10)
#cart
#dudoancarttheotest
y_predictCart = clfCart.predict(X_test_first)
lbl1 = Label(form)
lbl1.grid(column=1, row=26)
lbl1.configure(text="Tỉ lệ dự đoán đúng của CART: "+str(accuracy_score(y_test_first, y_predictCart)*100)+"%"+'\n'
...
#ID3
#dudoanid3test
y_predictID3 = clfID3.predict(X_test_first)
lbl3 = Label(form)
lbl3.grid(column=2, row=26)
lbl3.configure(text="Tỉ lệ dự đoán đúng của ID3: "+str(accuracy_score(y_test_first, y_predictID3)*100)+"%"+'\n'
...
#SVM
#duDoanSVM
y_svm = svm.predict(X_test_first)
lbl5 = Label(form)
lbl5.grid(column=3, row=26)
lbl5.configure(text="Tỉ lệ dự đoán đúng của SVM: "+str(accuracy_score(y_test_first, y_svm)*100)+"%"+'\n'
                           +"Precision: "+str(precision_score(y_test_first, y_svm)*100)+"%"+'\n'
                           +"Recall: "+str(recall_score(y_test_first, y_svm)*100)+"%"+'\n'
                           +"F1-score: "+str(f1_score(y_test_first, y_svm)*100)+"%"+'\n')

#khi dung PCA
lable_note = Label(form, text = "Khi dùng PCA :",font=("Arial Bold", 10),fg="red")
lable_note.grid(row = 27, column = 1,padx = 40, pady = 10)
#cart
#dudoancarttheotest
X_test_PCA_cart = pca_best_cart.transform(X_test_first)
y_cart_PCA = modelMax_cart.predict(X_test_PCA_cart)
lbl2 = Label(form)
lbl2.grid(column=1, row=28)
lbl2.configure(text="Tỉ lệ dự đoán đúng của CART: "+str(accuracy_score(y_test_first, y_cart_PCA)*100)+"%"+'\n'
                           +"Precision: "+str(precision_score(y_test_first, y_cart_PCA)*100)+"%"+'\n'
                           +"Recall: "+str(recall_score(y_test_first, y_cart_PCA)*100)+"%"+'\n'
                           +"F1-score: "+str(f1_score(y_test_first, y_cart_PCA)*100)+"%"+'\n')

# # #ID3
# # #dudoanid3test
X_test_PCA_id3 = pca_best_id3.transform(X_test_first)
y_id3_PCA = modelMax_id3.predict(X_test_PCA_id3)
lbl3 = Label(form)
lbl3.grid(column=2, row=28,padx = 40, pady = 10)
lbl3.configure(text="Tỉ lệ dự đoán đúng của ID3: "+str(accuracy_score(y_test_first, y_id3_PCA)*100)+"%"+'\n'
                           +"Precision: "+str(precision_score(y_test_first, y_id3_PCA)*100)+"%"+'\n'
                           +"Recall: "+str(recall_score(y_test_first, y_id3_PCA)*100)+"%"+'\n'
                           +"F1-score: "+str(f1_score(y_test_first, y_id3_PCA)*100)+"%"+'\n')

# # #SVM
# # #duDoanSVM
X_test_PCA_svm = pca_best_svm.transform(X_test_first)
y_svm_PCA = modelMax_svm.predict(X_test_PCA_svm)
lbl5 = Label(form)
lbl5.grid(column=3, row=28)
lbl5.configure(text="Tỉ lệ dự đoán đúng của SVM: "+str(accuracy_score(y_test_first, y_svm_PCA)*100)+"%"+'\n'
                           +"Precision: "+str(precision_score(y_test_first, y_svm_PCA)*100)+"%"+'\n'
                           +"Recall: "+str(recall_score(y_test_first, y_svm_PCA)*100)+"%"+'\n'
                           +"F1-score: "+str(f1_score(y_test_first, y_svm_PCA)*100)+"%"+'\n')
...
def gandulieucodoc():
    cleardata()
    textbox_1.insert(0,'x')
    textbox_2.insert(0,'y')
    textbox_3.insert(0,'w')
    textbox_4.insert(0,'t')
    textbox_5.insert(0,'p')
    textbox_6.insert(0,'f')
    textbox_7.insert(0,'c')
    textbox_8.insert(0,'n')
    textbox_9.insert(0,'n')
    textbox_10.insert(0,'e')
...
def dudoanID3():
    ...
    if((data1 == '') or (data2 == '') or (data3 == '') or (data4 == '') or (data5 == '') or (data6 == '') or(data7 == '') or (data8 == '') or (data9 == '') or (data10 == '') or (data11 == '') or (data12 == '') or (data13 == '') or (data14 == '') or (data15 == '') or (data16 == '') or (data17 == '') or (data18 == '') or (data19 == '') or (data20 == '') or (data21 == '') or (data22 == '')  ):
        lblvalue.configure(text= '....')
        messagebox.showinfo("Thông báo", "Bạn cần nhập đầy đủ thông tin!")
    else:
        df1 = pd.read_csv('./nam.csv') #đọc file csv
        check  = np.array(['p',data1,data2,data3,data4,data5,data6,data7,data8,data9,data10,data11,data12,data13,data14,data15,data16,data17,data18,data19,data20,data21,data22])
        print(check)
        ...
        df1 = df1.drop(np.shape(df)[0]-1)
        data1 = data1.drop(np.shape(data)[0]-1)
        X_id3_cart_ex = pca_best_id3.transform(datatrain)  
        y_kqua = modelMax_id3.predict(X_id3_cart_ex)
        if(y_kqua == 1):
            lblvalue.configure(text= 'Cây nấm có độc')
            cleardata()
        else:
            lblvalue.configure(text= 'Cây nấm KHÔNG có độc')
            cleardata()
# Button
button_cart1 = Button(form, text = 'Kết quả dự đoán theo ID3', command = dudoanID3)
button_cart1.grid(row = 3, column = 2, padx = 30)
lblvalue = Label(form, text="...")
lblvalue.grid(column=2, row=4)
button_cart1 = Button(form, text = 'set data', command = gandulieu)
button_cart1.grid(row = 5, column = 2, padx = 30)
button_cart1 = Button(form, text = 'set data', command = gandulieucodoc)
button_cart1.grid(row = 6, column = 2, padx = 30)
